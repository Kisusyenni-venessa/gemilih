<!DOCTYPE html>
<html lang="en">
<?php include_once("parts/head.php")?>

<body>

    <!-- Preloader -->
    <?php include_once("parts/loader.php")?>
    <!-- End of Preloader -->

    <!-- Main Header Start -->
    <?php include_once("parts/header.php")?>
    <!-- End of Main Header -->

    <!-- Page title -->
    <div class="lawFirm--page-title text-center lawFirm--bg-center" data-bg-img="assets/images/lokasi-img/header-bg.png">
        <div class="container">
            <h2 class="pb-20">News</h2>
            <div class="title-bc"> <a href="index.php">Home</a> > <span>News</span></div>
        </div>
    </div>
    <!-- End of Page title -->


    <div class="container pt-120 pb-120">
        <div class="row">

            <!-- News Sidebar Start -->
            <?php include_once("parts/news-sidebar.php")?>
            <!-- End of News Sidebar -->

            <div class="col-lg-9 col-md-8 order-1 order-md-2">
                <div class="lawFirm--blog-details">
                    <div class="post-details-cover">
                        <!-- Post Thumbnail -->
                        <div class="post-thumb-cover">
                            <!-- Post Meta Info -->
                            <div class="post-meta-info">
                                <!-- Category -->
                                <!-- Title -->
                                <div class="title">
                                    <h2>Halau Klaster KPU Batam, Wali Kota Teken Kerja Sama Protokol Kesehatan, Sekda Saran Usia Energik</h2>
                                </div>
                                <!-- Meta -->
                                <ul class="nav meta align-items-center">
                                    <li class="meta-author">
                                        <img src="assets/images/news-img/author/tribunbatam.jpg" alt=""
                                            class="img-fluid" data-rjs="2">
                                        <a href="https://batam.tribunnews.com">TribunBatam</a>
                                    </li>
                                    <li class="meta-date"><a href="news-5.php">Kamis, 9 Juli 2020 19:55 WIB</a></li>
                                    <li> 2 min read </li>
                                </ul>
                            </div>
                            <!-- End of Post Meta Info -->
                            <div class="post-thumb">
                                <img src="assets/images/news-img/news5.jpg" alt="" class="img-fluid" data-rjs="2">
                            </div>
                        </div>
                        <!-- End oF Post Thumbnail -->

                        <!-- Post Content -->
                        <div class="post-content-cover my-drop-cap">
                            <!-- <blockquote>
                                <p>"Jadi kami belum bisa menyebutkan hasil verifikasi faktual, karena masih berjalan",
                                </p>
                                <cite>Haruki Murakami</cite>
                            </blockquote> -->

                            <p>
                                Pesan itu ia sampaikan saat Rapat Pembahasan Persiapan Pilkada bersama bupati/wali kota serta perangkat Pemilu se-Kepri via video konferensi, Jumat (12/06/2020).
                            </p>

                            <p>
                                “Pilkada saat ini berbeda dengan tahun-tahun sebelumnya.
                            </p>

                            <p>
                                Pelantikan PPS di sejumlah kecamatan di Karimun, Senin (15/06/2020). KPU Karimun menggesa tahapan pilkada yang sempat tertunda (TRIBUNBATAM.ID/ELHADIF PUTRA)
                            </p>

                            <p>
                                Saran saya kalau bisa petugas di lapangan janganlah mereka yang umurnya rentan tertular, lebih baik yang masih energik,” ujarnya.
                            </p>

                            <p>
                                Menurut Arif, kalaupun yang betugas di lapangan berusia lanjut agar dipastikan kesehatannya.
                            </p>

                            <p>
                                “Karena pengalaman Pilkada sebelumnya banyak yang meninggal akbiat kelelahan, apalagi saat ini ada Covid-19 juga, tentunya sangat berbahaya,” harap Arif.
                            </p>

                            <p>
                                Berbicara terkait Pilkada di Kepri, ia mengatakan tujuh wilayah bersiap-siap menggelar pesta demokrasi di masa pandemi.
                            </p>

                            <p>
                                Setelah jadwalnya sempat diundur, pemerintah dan KPU kompak menggelar Pilkada Serentak pada 9 Desember 2020 nanti.
                            </p>

                            <p>
                                Berbicara terkait Pilkada di Kepri, ia mengatakan tujuh wilayah bersiap-siap menggelar pesta demokrasi di masa pandemi.
                            </p>

                            <p>
                                Setelah jadwalnya sempat diundur, pemerintah dan KPU kompak menggelar Pilkada Serentak pada 9 Desember 2020 nanti.
                            </p>

                            <div class="post-my-gallery-images">
                                <h3>The Best Neighborhoods In Nyc: Where To Stay On </h3>
                                <div class="row">
                                    <div class="col-md-6">
                                        <img src="assets/images/news-img/news5a.jpg" alt="" class="img-fluid mb-0" data-rjs="2"><br>
                                        <em>MoU - Wali Kota Batam M Rudi dan empat Komisioner KPU Batam usai penandatangani MoU pencegahan Covid-19 di Kantor Wali Kota Batam, Kamis (09/07/2020). (TRIBUN BATAM/BERES LUMBANTOBING)</em>
                                    </div>
                                    <div class="col-md-6">
                                        <img src="assets/images/news-img/news5b.jpg" alt="" class="img-fluid mb-0" data-rjs="2"><br>
                                        <em>Pelantikan PPS di sejumlah kecamatan di Karimun, Senin (15/06/2020). KPU Karimun menggesa tahapan pilkada yang sempat tertunda (TRIBUNBATAM.ID/ELHADIF PUTRA)</em>
                                    </div>
                                    <div class="col-md-12 mt-4">
                                        <img src="assets/images/news-img/news5c.jpg" alt="" class="img-fluid mb-0" data-rjs="2"><br>
                                        <em>Salah satu tukang parkir yang menolak memakai masker hingga bersitegang dengan Satgas Covid 19 Bandar Lampung, Kamis (09/07/2020) (Kompas.com) Kepri akan menggelar Pilkada mulai tingkat gubernur, wali kota hingga bupati.</em>
                                    </div>
                                </div>
                            </div>
                            
                            <p>
                                Hal ini ditandai dengan penandatanganan Memorandum Of Understanding (Mou) oleh Ketua Gugus Tugas Batam M Rudi dan KPU Batam Herrigen, di Balai Kota, Kamis (09/07/2020).
                            </p>

                            <p>
                                Herrigen mengatakan MoU bertujuan untuk memastikan kesehatan penyelenggara Pemilu tidak terpalar virus.
                                Selain itu, MoU untuk menjamin protokol kesehatan berjalan saat Pilkada 9 Desember mendatang.
                            </p>

                            <p>
                                "Setiap hari ketemu orang banyak sangat berbahaya. Kan, tidak baik, ada pula klaster Covid-19 KPU Batam, makanya sedini mungkin harus kita cegah," tegasnya.
                            </p>

                            <p>
                                Sebelumnya Sekretaris Daerah Provinsi (Sekdaprov) Kepulauan Riau (Kepri) TS Arif Fadillah, meminta seluruh penyelenggara Pemilu menjaga kesehatan dan mengingat hidup bersih.
                                Pesan itu ia sampaikan saat Rapat Pembahasan Persiapan Pilkada bersama bupati/wali kota serta perangkat Pemilu se-Kepri via video konferensi, Jumat (12/06/2020).
                            </p>

                            <p>
                                “Pilkada saat ini berbeda dengan tahun-tahun sebelumnya.
                                Saran saya kalau bisa petugas di lapangan janganlah mereka yang umurnya rentan tertular, lebih baik yang masih energik,” ujarnya.
                            </p>

                            <p>
                                Menurut Arif, kalaupun yang betugas di lapangan berusia lanjut agar dipastikan kesehatannya.
                            </p>

                            <p>
                                “Karena pengalaman Pilkada sebelumnya banyak yang meninggal akbiat kelelahan, apalagi saat ini ada Covid-19 juga, tentunya sangat berbahaya,” harap Arif.
                            </p>

                            <p>
                                Berbicara terkait Pilkada di Kepri, ia mengatakan tujuh wilayah bersiap-siap menggelar pesta demokrasi di masa pandemi.
                                Setelah jadwalnya sempat diundur, pemerintah dan KPU kompak menggelar Pilkada Serentak pada 9 Desember 2020 nanti.
                            </p>

                            <p>
                                Berbicara terkait Pilkada di Kepri, ia mengatakan tujuh wilayah bersiap-siap menggelar pesta demokrasi di masa pandemi.
                                Setelah jadwalnya sempat diundur, pemerintah dan KPU kompak menggelar Pilkada Serentak pada 9 Desember 2020 nanti.
                            </p>

                            <p>
                                Di Kepri, hanya Kota Tanjungpinang tak melaksanakan Pilkada, lantaran belum berakhirnya masa jabatan kepala daerah.
                            </p>

                            <p>
                                “Provinsi Kepri, Batam, Bintan, Karimun, Lingga, Natuna serta Anambas akan melaksanakan Pilkada pada 9 Desember.
                                Di Tanjungpinang hanya melaksanakan Pemilihan Gubernur,” jelasnya.
                            </p>


                            
                        </div>
                        <!-- End of Post Content -->

                        <!-- Tags -->
                        <div class="post-all-tags">
                            <a href="#">tribunbatam</a>
                            <a href="#">Tribun Batam</a>
                            <a href="#">Pilwako Batam</a>
                            <a href="#">Ketua KPU Batam</a>
                            <a href="#">Wali Kota Batam Muhammad Rudi</a>
                            <a href="#">Pilkada Serentak</a>
                            <a href="#">Wabah corona</a>
                            <a href="#">Pilkada saat pandemi Covid-19</a>
                            <a href="#">Covid-19</a>
                        </div>
                        <!-- End of Tags -->

                        <!-- -->

                        <!-- News Comment -->
                        <?php include_once("parts/news-comment.php")?>
                        <!-- End of News Comment -->

                    </div>
                </div>
            </div>
        </div>
    </div>


    <!-- Footer -->
    <?php include_once("parts/footer.php")?>
    <!-- End of Footer -->

    <!-- Back to Top Button -->
    <?php include_once("parts/back-to-top.php")?>
    <!-- End of Back to Top Button -->

    <!-- JS Files -->
    <?php include_once("parts/script.php")?>

</body>

</html>